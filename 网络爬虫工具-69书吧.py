...
async def download_chapter(session, chapter_url, chapter_title, save_dir, pbar, serial_num):
    """
    serial_num: 正确的章节序号，例如 1, 2, 3
    """
    safe_title = validate_filename(chapter_title)

    # 🟢 修复：文件名强制使用 5 位补零格式 (00001_xxxx.txt)
...
def post_process_files(temp_chapter_dir, book_title):
    ...
    # 🟢 关键：因为文件名现在是 00001_ 开头，直接 sort 就是正确的顺序
    all_files.sort()
    ...
    with open(merged_file_path, 'w', encoding='utf-8') as outfile:
        for filename in all_files:
            ...
            # 去掉文件名的序号前缀，只保留标题写入内容
            # 例如: "00001_第一章.txt" -> "第一章"
    ...
    async with AsyncSession(impersonate="chrome120") as session:
        try:
            ...
            if os.path.exists(final_txt_path):
                print(f"⏭️  跳过已下载: 《{safe_book_title}》")
                return
            ...
            if chapter_list:
                ...
                # 如果列表最后一个元素是 "第1章" 或 "第一章"，说明列表是倒序的
                # 或者，如果列表第一个元素包含很大的数字（比如第700章），也说明是倒序
                is_reversed = False
